defmodule ExBankingTest do
  ...
  describe "creating new user" do
    test ": with the name another user" do
    end
    test ": balance on every currency is 0" do
    end
  end
  ...
  describe "increasing user's balance" do
    ...
    test ": user and USER are not the same person", cnt do
      assert :user_does_not_exist = ExBanking.deposit(String.uppercase(cnt.user1.id), 1000, cnt.cur1)
    end
    ...
    test ": increase balance of one user isn't mean increasing balance of another user", %{user1: user1, user2: user2, cur2: cur} do
  ...
  describe "decreasing user's balance" do
    test ": want to send more than 10 requests to user", cnt do
      ExBanking.withdraw(cnt.user1, 1000, cnt.cur1)
    end
    ...
    test ": user and USER are not the same person", cnt do
      assert :user_does_not_exist == ExBanking.withdraw(String.uppercase(cnt.user1), 1000, cnt.cur1)
    end
    test ": balance depends on currency", %{user1: user, cur1: cur1, cur2: cur2, cur3: cur3} do
    ...
    test ": decrease balance of one user isn't mean decreasing balance of another user", %{user1: user1, user2: user2, cur2: cur} do
  ...
  describe "sending user's money" do
    setup cnt do
      {:ok, balance} = ExBanking.deposit(cnt.user3, 1000, cnt.cur3)
      put_in(cnt, [:user3, :balance, cur3], balance)
    end
    ...
    test ": want to send more amount then you have", cnt do
      assert :not_enough_money == ExBanking.send(cnt.user1.id, cnt.user2.id, cnt.user1.balance[cnt.cur1] + 1, cur1)
    end
    ...
    test ": user and USER are not the same person", cnt do
      assert :user_does_not_exist == ExBanking.send(cnt.user1.id, String.uppercase(cnt.user2.id), 1, cur1)
    end

    test ": succesful", cnt do
    end
  end
